...
export function calculateIntentScore(input: string, intent: Intent): number {
  ...
  uniquePatternWords.forEach(pw => {
    if (words.includes(pw)) {
      ...
      const nouns = ['lead', 'task', 'pipeline', 'workflow', 'automation', 'contact', 'weather', 'name', 'deal', 'roi', 'mao', 'profit', 'buyer', 'market', 'sqft', 'arv', 'offer', 'price'];
...
export async function recognizeIntent(input: string): Promise<ParsedIntent | null> {
  ...
  const exactKeywordMap: Record<string, string> = {
    ...
    'go to calendar': 'navigate_to',
    'open calendar': 'navigate_to',
    'go to leads': 'navigate_to',
    'open leads': 'navigate_to',
    'go to tasks': 'navigate_to',
    'open tasks': 'navigate_to',
    'go to sms': 'navigate_to',
    'open sms': 'navigate_to',
    'go to automations': 'navigate_to',
    'open automations': 'navigate_to',
    'go to dashboard': 'navigate_to',
    'open dashboard': 'navigate_to',
    'go to settings': 'navigate_to',
    'open settings': 'navigate_to',
    'go to buyers': 'navigate_to',
    'open buyers': 'navigate_to',

    // Pipeline
    'pipeline': 'pipeline_stats',
    'pipeline stats': 'pipeline_stats',
    'pipeline summary': 'pipeline_stats',
    'deal breakdown': 'pipeline_stats',

    // Briefing
    'brief me': 'daily_briefing',
    'morning summary': 'daily_briefing',
    'daily briefing': 'daily_briefing',
    'catch me up': 'daily_briefing',
    'whats happening': 'daily_briefing',
    "what's happening": 'daily_briefing',

    // Export
    'export data': 'export_data',
    'export leads': 'export_data',
    'download data': 'export_data',
    'download leads': 'export_data',

    // Automations
    'automations': 'navigate_to',

    // Follow-ups
    'stale leads': 'smart_followup',
    'cold leads': 'smart_followup',
    'neglected leads': 'smart_followup',
  };
  ...
  const handlers = [
    ...
    // ──── NEW INTELLIGENCE HANDLERS ────
    {
      intent: 'delete_task',
      patterns: [
        /^(?:delete|remove|cancel|drop)\s+(?:the\s+)?task\s+(?:called\s+|named\s+)?(.+)$/i,
        /^(?:delete|remove|cancel|drop)\s+(?:the\s+)?\"?(.+?)\"?\s+task$/i,
      ],
      params: (matches: string[]) => ({ target: matches[1]?.trim() })
    },
    {
      ...
      patterns: [
        /^(?:change|set|update|make)\s+(?:the\s+)?(?:priority\s+(?:of|for)\s+)?(.+?)\s+(?:to|as)\s+(high|medium|low|urgent)$/i,
        /^(?:make)\s+(?:the\s+)?(.+?)\s+(?:task\s+)?(urgent|high priority|low priority)$/i,
        /^(?:change|set|update)\s+(?:the\s+)?task\s+(.+?)\s+(?:priority\s+)?(?:to|as)\s+(high|medium|low|urgent)$/i,
      ],
      params: (matches: string[]) => ({ target: matches[1]?.trim(), priority: matches[2]?.trim()?.toLowerCase() })
    },
    {
      intent: 'delete_lead',
      patterns: [
        /^(?:delete|remove|drop|erase)\s+(?:the\s+)?lead\s+(?:for\s+|named\s+|called\s+)?(.+)$/i,
        /^(?:delete|remove|drop|erase)\s+(.+?)\s+(?:from\s+)?(?:leads|lead|my leads|crm|database)$/i,
      ],
      params: (matches: string[]) => ({ target: matches[1]?.trim() })
    },
    {
      intent: 'navigate_to',
      patterns: [
        /^(?:go to|take me to|open|navigate to|show me|switch to)\s+(?:the\s+)?(.+?)(?:\s+page)?$/i,
      ],
      params: (matches: string[]) => ({ page: matches[1]?.trim()?.toLowerCase() })
    },
    {
      intent: 'pipeline_stats',
      patterns: [
        /^(?:pipeline|pipeline stats|pipeline summary|deal breakdown|show pipeline|conversion rate|how are my deals)$/i,
        /^(?:how many deals|deals this month|deal status|lead breakdown)$/i,
      ],
      params: () => ({})
    },
    {
      intent: 'calendar_query_ext',
      patterns: [
        /^(?:what's on my calendar|whats on my calendar|my calendar|show my calendar|any meetings today|my agenda|today's agenda|schedule today|show my schedule)$/i,
        /^(?:do i have any meetings|any appointments|what do i have scheduled|what's my schedule)$/i,
      ],
      params: () => ({})
    },
    {
      intent: 'add_buyer',
      patterns: [
        /^(?:add|create|new)\s+(?:a\s+)?buyer\s+(?:named?\s+)?(.+?)(?:\s+(?:for|in|looking in)\s+(.+))?$/i,
        /^(?:register)\s+(.+?)\s+as\s+(?:a\s+)?buyer$/i,
      ],
    ...
    },
    {
      intent: 'search_crm',
      patterns: [
        /^(?:search|search for|find|look up|look for)\s+(.+?)(?:\s+in\s+(?:leads|tasks|crm|buyers|contacts))?$/i,
      ],
      params: (matches: string[]) => ({ query: matches[1]?.trim() })
    },
    {
      intent: 'bulk_action',
      patterns: [
        /^(?:text|message)\s+(?:all|every)\s+(.+?)(?:\s+saying\s+(.+))?$/i,
        /^(?:mark|set)\s+all\s+(.+?)\s+(?:as\s+)?(.+)$/i,
      ],
      params: (matches: string[]) => ({ group: matches[1]?.trim(), action_data: matches[2]?.trim() })
    },
    {
      intent: 'daily_briefing',
      patterns: [
        /^(?:brief me|morning summary|daily briefing|daily summary|catch me up|morning briefing|give me the rundown|rundown)$/i,
        /^(?:what's happening|whats happening|what's going on|whats going on|what did i miss|update me)$/i,
      ],
      params: () => ({})
    },
    {
      intent: 'export_data',
      patterns: [
        /^(?:export|download|backup)\s+(?:my\s+)?(?:leads|data|crm|contacts|tasks|everything)$/i,
      ],
      params: () => ({})
    },
    {
      intent: 'set_reminder',
      patterns: [
        /^(?:remind me|reminder|alert me|notify me)\s+(?:in\s+)?(\d+)\s+(minutes?|hours?|days?)\s+(?:to\s+)?(.+)$/i,
        /^(?:remind me|reminder)\s+(?:to\s+)(.+?)\s+(?:in\s+)(\d+)\s+(minutes?|hours?|days?)$/i,
        /^(?:set a reminder|create reminder)\s+(?:for\s+|to\s+)?(.+?)(?:\s+in\s+(\d+)\s+(minutes?|hours?|days?))?$/i,
      ],
      params: (matches: string[]) => {
        // Handle both orderings
        if (/^\d+$/.test(matches[1])) {
          return { amount: parseInt(matches[1]), unit: matches[2]?.replace(/s$/, ''), task: matches[3]?.trim() };
        }
        return { task: matches[1]?.trim(), amount: parseInt(matches[2] || '1'), unit: (matches[3] || 'hour')?.replace(/s$/, '') };
      }
    },
    {
      intent: 'count_query',
      patterns: [
        /^(?:how many)\s+(leads?|tasks?|buyers?|contacts?|deals?|sms|messages?)\s*(?:do i have)?$/i,
        /^(?:lead|task|buyer|contact|deal)\s+count$/i,
        /^(?:total)\s+(leads?|tasks?|buyers?)$/i,
      ],
      params: (matches: string[]) => ({ entity: matches[1]?.trim()?.toLowerCase()?.replace(/s$/, '') })
    },
    {
      intent: 'toggle_automation',
      patterns: [
        /^(?:turn on|enable|activate)\s+(?:the\s+)?(.+?)$/i,
        /^(?:turn off|disable|deactivate)\s+(?:the\s+)?(.+?)$/i,
      ],
      params: (matches: string[]) => {
        const full = (matches[0] || '').toLowerCase();
        const enable = /^(?:turn on|enable|activate)/i.test(full);
        return { feature: matches[1]?.trim()?.toLowerCase(), enable };
      }
    },
    {
      intent: 'smart_followup',
      patterns: [
        /^(?:who needs a follow up|who do i need to follow up with|follow up list|who needs attention)$/i,
        /^(?:stale leads|cold leads|neglected leads|who havent i talked to|inactive leads)$/i,
        /^(?:which leads need follow up|leads to call|who should i call)$/i,
      ],
      params: () => ({})
    },
    {
      intent: 'rei_knowledge',
      patterns: [
        /^(?:what is|explain|define|how does|what do you mean by)\s+(?:a\s+|an\s+|the\s+)?(.+?)\s*(?:\?|work|mean)?$/i,
        /^(?:whats|what's|what are)\s+(?:a\s+|an\s+|the\s+)?(.+?)\s*(?:\?|work|mean)?$/i,
      ],
      params: (matches: string[]) => ({ concept: matches[1]?.trim()?.toLowerCase() })
    },
    {
      intent: 'analyze_deal',
      patterns: [
        /^(?:analyze|evaluate|run the numbers on)\s+(?:the\s+)?(.+?)(?:\s+deal)?$/i,
        /^(?:calculate|what is the)\s+(?:arv|mao|roi|breakdown)\s+(?:for\s+)?(.+?)$/i,
      ],
      params: (matches: string[]) => ({ target: matches[1]?.trim() })
    },
    {
      ...
      patterns: [
        /^(?:write|draft|generate|create)\s+(?:an?\s+)?(sms|text|message|email|script|cold call script)\s+(?:to|for)\s+(.+?)(?:\s+saying\s+(.+))?$/i,
        /^(?:what should i say to)\s+(.+)$/i,
      ],
      params: (matches: string[]) => {
        // Handle "what should i say to"
        if (matches[0].toLowerCase().startsWith('what should i say')) {
           return { type: 'script', target: matches[1]?.trim() };
        }
        return { type: matches[1]?.trim()?.toLowerCase(), target: matches[2]?.trim(), parameters: matches[3]?.trim() };
      }
    },
    {
      intent: 'market_intelligence',
      patterns: [
        /^(?:market temperature|market stats|buyer matches|pipeline insights)\s*(?:for\s+)?(.*)?$/i,
        /^(?:who are my best buyers|best buyers)\s*(?:for\s+)?(.*)?$/i,
      ],
      params: (matches: string[]) => ({ location: matches[1]?.trim()?.toLowerCase() })
    }
  ];
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // STAGE 4.5: VERB-NOUN DECOMPOSITION (NLU)
  // Parse natural language into verb + object pairs for smart intent routing.
  // Example: "delete the call john task" → verb=delete, object=task
  // ═══════════════════════════════════════════════════════════════════════
  if (wordCount >= 2) {
    const verbNounMap: Record<string, Record<string, string>> = {
      'delete': { 'task': 'delete_task', 'lead': 'delete_lead' },
      'remove': { 'task': 'delete_task', 'lead': 'delete_lead' },
      'cancel': { 'task': 'delete_task' },
      'drop':   { 'task': 'delete_task', 'lead': 'delete_lead' },
      'erase':  { 'lead': 'delete_lead' },
      'add':    { 'buyer': 'add_buyer', 'task': 'add_task', 'lead': 'create_lead' },
      'create': { 'buyer': 'add_buyer', 'task': 'add_task', 'lead': 'create_lead' },
      'new':    { 'buyer': 'add_buyer', 'task': 'add_task', 'lead': 'create_lead' },
      'show':   { 'calendar': 'navigate_to', 'leads': 'list_leads', 'tasks': 'show_tasks', 'pipeline': 'pipeline_stats', 'buyers': 'navigate_to' },
      'open':   { 'calendar': 'navigate_to', 'leads': 'navigate_to', 'tasks': 'navigate_to', 'sms': 'navigate_to', 'settings': 'navigate_to', 'dashboard': 'navigate_to', 'automations': 'navigate_to', 'buyers': 'navigate_to' },
      'count':  { 'lead': 'count_query', 'task': 'count_query', 'buyer': 'count_query', 'deal': 'count_query' },
      'export': { 'lead': 'export_data', 'data': 'export_data', 'everything': 'export_data' },
      'search': { 'lead': 'search_crm', 'crm': 'search_crm', 'buyer': 'search_crm', 'task': 'search_crm' },
      'find':   { 'lead': 'search_crm', 'buyer': 'search_crm', 'task': 'search_crm' },
    };
    ...
    if (verbNounMap[verb]) {
      // Look for a matching noun in the rest of the input
      for (const [noun, intentName] of Object.entries(verbNounMap[verb])) {
        if (words.includes(noun) || words.includes(noun + 's')) {
          const intentObj = intents.find(i => i.name === intentName);
          if (intentObj) {
            // Extract the qualifier (everything that's not the verb or noun)
            const qualifier = words.filter(w => w !== verb && w !== noun && w !== noun + 's' && w !== 'the' && w !== 'a' && w !== 'my').join(' ').trim();
            debugLog('MATCHED', `Verb-noun decomposition: verb="${verb}" + noun="${noun}" → ${intentName}`, { qualifier });
            return {
              intent: intentObj,
              params: { target: qualifier || undefined, page: noun, entity: noun },
              confidence: 90,
              needsAgentLoop,
              matchedBy: `verb_noun:${verb}_${noun}`
            };
          }
        }
      }
