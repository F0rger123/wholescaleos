      ...
      try {
        ...
        const [
          ...
          Promise.all([
            ...
            supabase!.from('timeline_entries').select('*').eq('team_id', teamId).order('created_at', { ascending: true }),
            supabase!.from('status_history').select('*').eq('team_id', teamId).order('changed_at', { ascending: true }),
          ]),
          fetchTimeout
        ]) as any;
