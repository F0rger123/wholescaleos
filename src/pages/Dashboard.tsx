...
const statusBarColors: Record<string, string> = {
  new: 'bg-[var(--t-info)]', contacted: 'bg-[var(--t-warning)]', qualified: 'bg-[var(--t-accent)]',
  negotiating: 'bg-[var(--t-accent)]', 'closed-won': 'bg-[var(--t-success)]', 'closed-lost': 'bg-[var(--t-error)]',
};

const SOURCE_COLORS: Record<string, { bg: string; text: string; bar: string; label: string }> = {
  website: { bg: 'bg-[var(--t-info)]/15', text: 'text-[var(--t-info)]', bar: 'bg-[var(--t-info)]', label: 'Website' },
  referral: { bg: 'bg-[var(--t-success)]/15', text: 'text-[var(--t-success)]', bar: 'bg-[var(--t-success)]', label: 'Referral' },
  'cold-call': { bg: 'bg-[var(--t-primary)]/15', text: 'text-[var(--t-primary)]', bar: 'bg-[var(--t-primary)]', label: 'Cold Call' },
  'social-media': { bg: 'bg-[var(--t-secondary)]/15', text: 'text-[var(--t-secondary)]', bar: 'bg-[var(--t-secondary)]', label: 'Social Media' },
  mailer: { bg: 'bg-[var(--t-warning)]/15', text: 'text-[var(--t-warning)]', bar: 'bg-[var(--t-warning)]', label: 'Mailer' },
  other: { bg: 'bg-[var(--t-text-muted)]/15', text: 'text-[var(--t-text-muted)]', bar: 'bg-[var(--t-text-muted)]', label: 'Other' },
};
              ...
              return (
                <div key={stage.key} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-8 bg-[var(--t-surface-hover)] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-xs font-bold text-[var(--t-text)]">{count}</span>
                    </div>
                  </div>
