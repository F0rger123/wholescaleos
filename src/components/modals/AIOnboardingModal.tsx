...
export function AIOnboardingModal({ isOpen, onClose, onConfigureCloud }: AIOnboardingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[var(--z-modal)] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div 
        className="bg-[var(--t-surface)] border border-[var(--t-border)] rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden"
      ...
      >
        ...
        <div className="p-6 md:p-8 relative">
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-2 text-[var(--t-text-muted)] hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full mb-8">
              ...
              <div className="p-4 rounded-xl border border-[var(--t-primary)]/20 bg-[var(--t-primary)]/5 text-left">
                <div className="flex items-center gap-2 mb-2">
                  <Brain className="w-4 h-4 text-white" />
                  <span className="text-sm font-bold text-white uppercase tracking-wider">Premium Cloud</span>
                </div>
                <ul className="text-xs text-[var(--t-text-muted)] space-y-2">
                  <li className="flex items-start gap-2">
                    <Sparkles className="w-3 h-3 mt-0.5 text-[var(--t-primary)]" />
                    GPT-4o, Claude 3.5
                  </li>
                  <li className="flex items-start gap-2">
                    <Zap className="w-3 h-3 mt-0.5 text-[var(--t-primary)]" />
                    Advanced Reasoning
                  </li>
                </ul>
              </div>
