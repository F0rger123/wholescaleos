import { supabase } from '../supabase';

// Types
export interface LearnedIntent {
  id?: string;
  user_id: string;
  phrase: string;
  mapped_intent: string;
  params?: Record<string, any>;
  confidence?: number;
  usage_count?: number;
  last_used?: string;
}

export interface ConversationMemory {
  id?: string;
  user_id: string;
  session_id: string;
  messages: Array<{role: 'user' | 'assistant', content: string, timestamp: string}>;
  active_topic?: string;
  mentioned_leads?: string[];
  mentioned_tasks?: string[];
}

export interface BotUserPreferences {
  user_id: string;
  preferred_name?: string;
  communication_style?: string;
  working_hours_start?: string;
  working_hours_end?: string;
  favorite_phrases?: string[];
  custom_responses?: Record<string, string>;
  remembered_facts?: Record<string, any>;
}

// Learned Intents
export async function saveLearnedIntent(
  userId: string,
  phrase: string,
  mappedIntent: string,
  params: Record<string, any> = {}
): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('learned_intents')
      .upsert({
        user_id: userId,
        phrase: phrase.toLowerCase().trim(),
        mapped_intent: mappedIntent,
        params,
        usage_count: 1,
        last_used: new Date().toISOString()
      }, {
        onConflict: 'user_id, phrase',
        ignoreDuplicates: false
      });

    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Failed to save learned intent:', e);
    return false;
  }
}

export async function getLearnedIntent(
  userId: string,
  phrase: string
): Promise<{ mapped_intent: string; params: Record<string, any>; confidence: number } | null> {
  try {
    const { data, error } = await supabase
      .from('learned_intents')
      .select('mapped_intent, params, confidence, usage_count')
      .eq('user_id', userId)
      .eq('phrase', phrase.toLowerCase().trim())
      .single();

    if (error || !data) return null;

    // Increment usage count
    await supabase
      .from('learned_intents')
      .update({ 
        usage_count: (data.usage_count || 1) + 1,
        last_used: new Date().toISOString()
      })
      .eq('user_id', userId)
      .eq('phrase', phrase.toLowerCase().trim());

    return {
      mapped_intent: data.mapped_intent,
      params: data.params || {},
      confidence: data.confidence || 100
    };
  } catch (e) {
    console.error('Failed to get learned intent:', e);
    return null;
  }
}

export async function getAllLearnedIntents(userId: string): Promise<LearnedIntent[]> {
  try {
    const { data, error } = await supabase
      .from('learned_intents')
      .select('*')
      .eq('user_id', userId)
      .order('last_used', { ascending: false })
      .limit(50);

    if (error) throw error;
    return data || [];
  } catch (e) {
    console.error('Failed to get learned intents:', e);
    return [];
  }
}

export async function deleteLearnedIntent(userId: string, phrase: string): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('learned_intents')
      .delete()
      .eq('user_id', userId)
      .eq('phrase', phrase.toLowerCase().trim());

    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Failed to delete learned intent:', e);
    return false;
  }
}

// Conversation Memory
export async function saveConversationTurn(
  userId: string,
  sessionId: string,
  role: 'user' | 'assistant',
  content: string
): Promise<void> {
  try {
    const newMessage = {
      role,
      content,
      timestamp: new Date().toISOString()
    };

    // Get existing session
    const { data: existing } = await supabase
      .from('conversation_memory')
      .select('messages')
      .eq('user_id', userId)
      .eq('session_id', sessionId)
      .single();

    let messages = existing?.messages || [];
    if (typeof messages === 'string') messages = JSON.parse(messages);
    
    messages.push(newMessage);

    // Keep only last 50 messages
    if (messages.length > 50) {
      messages = messages.slice(-50);
    }

    await supabase
      .from('conversation_memory')
      .upsert({
        user_id: userId,
        session_id: sessionId,
        messages,
        updated_at: new Date().toISOString()
      });
  } catch (e) {
    console.error('Failed to save conversation turn:', e);
  }
}

export async function getConversationContext(
  userId: string,
  sessionId: string,
  limit: number = 10
): Promise<Array<{role: 'user' | 'assistant', content: string}>> {
  try {
    const { data, error } = await supabase
      .from('conversation_memory')
      .select('messages')
      .eq('user_id', userId)
      .eq('session_id', sessionId)
      .single();

    if (error || !data) return [];

    let messages = data.messages;
    if (typeof messages === 'string') messages = JSON.parse(messages);
    
    return (messages || []).slice(-limit);
  } catch (e) {
    console.error('Failed to get conversation context:', e);
    return [];
  }
}

export async function updateConversationTopic(
  userId: string,
  sessionId: string,
  topic: string
): Promise<void> {
  try {
    await supabase
      .from('conversation_memory')
      .update({ active_topic: topic, updated_at: new Date().toISOString() })
      .eq('user_id', userId)
      .eq('session_id', sessionId);
  } catch (e) {
    console.error('Failed to update conversation topic:', e);
  }
}

export async function addMentionedLead(
  userId: string,
  sessionId: string,
  leadName: string
): Promise<void> {
  try {
    const { data } = await supabase
      .from('conversation_memory')
      .select('mentioned_leads')
      .eq('user_id', userId)
      .eq('session_id', sessionId)
      .single();

    const leads = data?.mentioned_leads || [];
    if (!leads.includes(leadName)) {
      leads.push(leadName);
      await supabase
        .from('conversation_memory')
        .update({ 
          mentioned_leads: leads.slice(-10),
          updated_at: new Date().toISOString()
        })
        .eq('user_id', userId)
        .eq('session_id', sessionId);
    }
  } catch (e) {
    console.error('Failed to add mentioned lead:', e);
  }
}

// User Preferences
export async function getUserPreferences(userId: string): Promise<BotUserPreferences | null> {
  try {
    const { data, error } = await supabase
      .from('bot_user_preferences')
      .select('*')
      .eq('user_id', userId)
      .single();

    if (error) return null;
    return data;
  } catch (e) {
    console.error('Failed to get user preferences:', e);
    return null;
  }
}

export async function saveUserPreference(
  userId: string,
  key: keyof BotUserPreferences,
  value: any
): Promise<boolean> {
  try {
    const update: Record<string, any> = { [key]: value };
    const { error } = await supabase
      .from('bot_user_preferences')
      .upsert({
        user_id: userId,
        ...update,
        updated_at: new Date().toISOString()
      });

    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Failed to save user preference:', e);
    return false;
  }
}

export async function rememberFact(
  userId: string,
  key: string,
  value: any
): Promise<boolean> {
  try {
    const { data } = await supabase
      .from('bot_user_preferences')
      .select('remembered_facts')
      .eq('user_id', userId)
      .single();

    const facts = data?.remembered_facts || {};
    facts[key] = value;

    const { error } = await supabase
      .from('bot_user_preferences')
      .upsert({
        user_id: userId,
        remembered_facts: facts,
        updated_at: new Date().toISOString()
      });

    if (error) throw error;
    return true;
  } catch (e) {
    console.error('Failed to remember fact:', e);
    return false;
  }
}

export async function getRememberedFact(userId: string, key: string): Promise<any | null> {
  try {
    const { data } = await supabase
      .from('bot_user_preferences')
      .select('remembered_facts')
      .eq('user_id', userId)
      .single();

    return data?.remembered_facts?.[key] || null;
  } catch (e) {
    return null;
  }
}

// Session Management
export function generateSessionId(): string {
  return `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}

// Check for similar phrases (fuzzy matching for learning)
export function findSimilarLearnedPhrase(
  phrase: string,
  learnedIntents: LearnedIntent[]
): LearnedIntent | null {
  const normalized = phrase.toLowerCase().trim();
  
  for (const intent of learnedIntents) {
    const learnedPhrase = intent.phrase.toLowerCase();
    
    // Exact match
    if (learnedPhrase === normalized) return intent;
    
    // Contains match
    if (learnedPhrase.includes(normalized) || normalized.includes(learnedPhrase)) {
      return intent;
    }
    
    // Word overlap > 70%
    const learnedWords = learnedPhrase.split(/\s+/);
    const phraseWords = normalized.split(/\s+/);
    const overlap = learnedWords.filter(w => phraseWords.includes(w)).length;
    const maxLength = Math.max(learnedWords.length, phraseWords.length);
    
    if (maxLength > 0 && overlap / maxLength >= 0.7) {
      return intent;
    }
  }
  
  return null;
}
