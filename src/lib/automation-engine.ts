...
class AutomationEngine {
  ...
  private matchesTrigger(node: Node, eventType: AutomationEventType, data: any): boolean {
    ...
    switch (eventType) {
      case 'LEAD_CREATED': return config.triggerType === 'new_lead';
      case 'TASK_STATUS_CHANGED': return config.triggerType === 'task_overdue' && data.status === 'overdue';
      case 'LEAD_STATUS_CHANGED': return config.triggerType === 'status_change' && data.status === config.status;
      case 'LEAD_SCORE_HIGH': return config.triggerType === 'high_score' && Number(data.deal_score || data.dealScore || 0) >= Number(config.threshold || 80);
      case 'EMAIL_OPENED': return config.triggerType === 'email_opened';
      case 'SMS_RECEIVED': return config.triggerType === 'sms_received';
      case 'DEAL_WON': return config.triggerType === 'status_change' && data.status === 'closed-won';
      case 'LEAD_INACTIVE': return config.triggerType === 'inactivity';
      case 'CALENDAR_EVENT_CREATED': return config.triggerType === 'new_event';
      case 'DOCUMENT_SIGNED': return config.triggerType === 'doc_signed';
      case 'SCHEDULED_MONTHLY': return config.triggerType === 'monthly_run';
      case 'UNSUBSCRIBED': return config.triggerType === 'unsubscribed';
      case 'FACEBOOK_LEAD': return config.triggerType === 'facebook_lead';
  ...
  private async runNodeAction(node: Node, context: any) {
    const config = node.data;
    console.log(`[AutomationEngine] Running action: ${config.type} (${config.label})`);

    try {
      switch (config.actionType) {
        case 'send_sms':
          console.log(`[AutomationEngine] Sending SMS to ${context.phone || 'unknown'}: ${config.message}`);
          break;

        case 'send_email':
          console.log(`[AutomationEngine] Sending Email to ${context.email || 'unknown'}: ${config.subject}`);
          // Integration with email service would go here
          break;

        case 'notify': {
          const userId = (context.user_id as string) || (context.assigned_to as string);
          if (userId) {
            await notificationsService.create({
              ...
              link: context.id ? `/leads/${context.id}` : undefined
            });
          }
          break;
        }
        ...
        case 'assign_lead':
          if (context.id && config.agentId) {
            await leadsService.update(context.id, { assigned_to: config.agentId });
          }
          break;

        case 'update_status':
          if (context.id && config.status) {
            await leadsService.update(context.id, { status: config.status });
          }
          break;
        ...
        case 'add_task':
          await tasksService.create({
            ...
            assigned_to: (config.agentId as string) || (context.assigned_to as string),
            lead_id: context.id,
